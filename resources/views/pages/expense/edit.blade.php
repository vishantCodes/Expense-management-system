...
@section('app-content')
    ...
    <form action="{{ route('admin.expense.update', ['expense' => $expense->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="row mb-3 border rounded-2 p-2">
                ...
                <div class="col-lg-3 col-md-6 col-sm-12">
                    ...
                    <select class="form-select @error('expense_request_type_id') is-invalid @enderror"
                        name="expense_request_type_id"
                        id="expense_request_type_id">
                        <option value="">Select Request Type</option>
                        @foreach ($requestTypeOptions as $type)
                            <option value="{{ $type->id }}"
                                {{ old('expense_request_type_id', $expense->expense_request_type_id ?? '') == $type->id ? 'selected' : '' }}>
                                {{ $type->label }}
                            </option>
                        @endforeach
                    </select>
                    @error('expense_request_type_id')
                    ...
                    <select class="form-select @error('department_id') is-invalid @enderror"
                        ...
                        @foreach ($departments as $department)
                            <option value="{{ $department->id }}"
                                {{ old('department_id', $expense->department_id ?? '') == $department->id ? 'selected' : '' }}>
                                {{ $department->title }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3 border rounded-2 p-2">
                ...
                <div class="col-lg-4 col-md-4 col-sm-12">
                    ...
                    <textarea class="form-control @error('justification') is-invalid @enderror"
                        ...
                        rows="3">{{ old('justification', $expense->justification ?? '') }}</textarea>
                ...
                <div class="col-lg-4 col-md-4 col-sm-12 mt-2">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input @error('is_pre_authorized') is-invalid @enderror"
                            ...
                            {{ old('is_pre_authorized', $expense->is_pre_authorized ?? '0') == '1' ? 'checked' : '' }}>
                        <label class="form-check-label" for="preAuthorizedYes">Yes</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio"
                            name="is_pre_authorized" id="preAuthorizedNo" value="0"
                            {{ old('is_pre_authorized', $expense->is_pre_authorized ?? '0') == '0' ? 'checked' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-lg-6 col-md-12 col-sm-12">
                            ...
                            <select class="form-select @error('payment_method') is-invalid @enderror"
                                ...
                                @foreach (['any' => 'Any', 'online' => 'Online', 'cash' => 'Cash', 'cheque' => 'Cheque'] as $val => $label)
                                    <option value="{{ $val }}"
                                        {{ old('payment_method', $expense?->payment_method?->value ?? $expense->payment_method ?? '') == $val ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-6 col-md-12 col-sm-12">
                            ...
                            <input class="form-control @error('payment_date') is-invalid @enderror"
                                ...
                                value="{{ old('payment_date', $expense?->payment_date ? $expense?->payment_date : '') }}">
